...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Top-K outfit compatibility retrieval.")
    p.add_argument(
        "--mode",
        required=True,
        choices=["top2bottom", "bottom2top"],
        help="Retrieval direction",
    )
    p.add_argument("--query-image", required=True, help="Path to query image")
    p.add_argument("--top-k", type=int, default=5, help="How many final results to return")
    p.add_argument(
        "--shortlist-k",
        type=int,
        default=150,
        help="Shortlist size before fused reranking",
    )
    p.add_argument(
        "--config",
...
def main() -> None:
    ...
    payload = [r.to_public_dict() if args.public_output else r.to_dict() for r in rows]
